            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-3">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-12">Sub Tests</h2>
                            <div class="d-flex align-items-center">
                                <button type="button" class="btn btn-sm btn-primary ml-10 mb-12" @click="addEditModalBtn('add','')" v-if="Helpers.hasPermission('test_management','can_add')">
                                    Add New
                                </button>
                            </div>
                        </div>
            ...
            <div class="content-body" ref="loadingRefContainer">
                <section class="app-user-list">
                    <div class="card">
                        ...
                        <div v-if="Helpers.hasPermission('test_management','can_view')">
                                        ...
                                        <span v-for="(item,index) in statusesCount" :key="index">
                                            <span v-if="item.count > 0">
                                                ...
                                                <router-link 
                                                    ...
                                                    :class="item.class"
                                                    :to="Helpers.appEndPoint()+'/sub-tests/'+mainTestId+'/'+item.key">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td></td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm" @input.prevent="getItems()" v-model="search.test_code">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm" @input.prevent="getItems()" v-model="search.test_name">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm" @input.prevent="getItems()" v-model="search.test_rate">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm" @input.prevent="getItems()" v-model="search.reporting_time">
                                            </td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                    ...
                                    <draggable 
                                        ...
                                        <template #item="{ element }">
                                            <tr>
                                                ...
                                                <td>
                                                    {{element.reporting_time}} <span v-if="element.reporting_time">Hours</span>
                                                </td>
                                                <td>
                                                    <span v-for="(subTestDetail,subIndex) in element.sub_test_details" :key="subIndex" class="badge cursor-pointer mr-4 mt-2 mb-2" v-bind:class="(subTestDetail.status == 'active')?'bg-primary':'bg-secondary'">
                                                        <span v-if="subTestDetail.gender">{{subTestDetail.gender.title}}</span>
                                                        <span v-if="subTestDetail.human_lifespan">/ {{subTestDetail.human_lifespan.title}}</span> ({{subTestDetail.normal_value}})
                                                    </span>
                                                </td>
                                                <td class="statusTd">
                                                ...
                                                <td class="actionTd" v-if="
                                                    Helpers.hasPermission('test_management','can_update') ||
                                                    Helpers.hasPermission('test_management','can_delete')
                                                ">
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                                            <router-link 
                                                                ...
                                                                :to="Helpers.appEndPoint()+'/sub-tests-details/'+mainTestId+'/'+element.id">
                                                                <span>List ({{ element.sub_test_details.length }})</span>
                                                            </router-link>
                                                            
                                                            <div class="dropdown-divider"></div>

                                                            <button @click="addEditModalBtn('edit',element.id)"
                                                            v-if="element.status != 'trash' && Helpers.hasPermission('test_management','can_update')" class="dropdown-item icon-vertical-center"><vue-feather class="icon" type="file-text" size="16"></vue-feather>Edit</button>
                                                            <button @click="deleteBtn(element.id)" v-if="Helpers.hasPermission('test_management','can_delete')" href="javascript:;" class="dropdown-item icon-vertical-center delete-record"><vue-feather class="icon" type="trash" size="16"></vue-feather>
...
import SubTestsModal from "@/components/modals/SubTestsModal.vue";
...
export default defineComponent({
    ...
    setup(){
        ...
        const mainTestId = ref(route.params.mainTestId)
